...
const refId = arguments[0];
const testId = arguments[1];

const referenceFile = document.getElementById(refId);
const testFile = document.getElementById(testId);
...
resemble(referenceFile).compareTo(testFile).onComplete(function(data) {
  ...
  const result = {data: data, diff: isDiff ?  data.getBuffer() : null};
  console.log(result);
});
